...
int BigFloat::CompareNum(const BigFloat& left, const BigFloat& right) {
  if ((left.number.size() - left.decimals) >
      (right.number.size() - right.decimals))
    return 1;
  else if ((left.number.size() - left.decimals) <
           (right.number.size() - right.decimals))
    return 2;
...
BigFloat BigFloat::Sum(const BigFloat& left, const BigFloat& right) {
  BigFloat tmp;
  int carry = 0;
  int loopsize = (left.number.size() > right.number.size())
                     ? left.number.size()
                     : right.number.size();
  ...
  if (carry != 0) tmp.number.push_back(IntToChar(carry));
...
BigFloat& BigFloat::operator=(long long Num) {
  *this = std::to_string(Num);
  return *this;
}
...
BigFloat operator/(const BigFloat& left, const BigFloat& right) {
  ...
  int div_precision =
      (left.decimals > right.decimals) ? (left.decimals) : (right.decimals);
  for (int i = 0; i < div_precision; i++) N.number.push_front('0');
...
BigFloat BigFloat::PrecDiv(const BigFloat& left, const BigFloat& right,
                           int div_precision) {
  BigFloat tmp;
...
BigFloat BigFloat::PrecDiv(const BigFloat& left, const int& int_right,
                           int div_precision) {
  BigFloat right;
  right = int_right;
...
BigFloat BigFloat::PrecDiv(const BigFloat& left, const double& double_right,
                           int div_precision) {
  BigFloat right;
  right = double_right;
  return BigFloat::PrecDiv(left, right, div_precision);
...
BigFloat operator%(const BigFloat& left, const BigFloat& right) {
  ...
  if (!BigFloat::CompareNum(tmp, zero)) tmp.sign = '+';
...
bool BigFloat::operator==(const BigFloat& right) const {
  if ((error) || (right.error)) return false;

  int check = CompareNum(*this, right);
  if ((check == 0) && (sign == right.sign)) return true;
  return false;
};
